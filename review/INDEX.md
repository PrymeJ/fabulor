...
| `Investigation_260802_double_fire_reentrancy.md` | 08-02 | Investigation | Root-caused the gutter-dismiss double-apply: `snap_theme_forward`'s fallback is gated on `_fade_overlay.isVisible()`, which `.stop()` never clears (only `_on_fade_finished` does, and `.stop()` emits no `finished`) — 10/12 in a real session log. **Cost-only, not wrong-final-state.** Built an independently-verified live harness (pixel + stylesheet ground truth, never internal flags); found and fixed a harness bug first (sample point `(15,15)` landed on a title-bar glyph, not `bg_main` — caused a uniform 100% false-fail). Two corrected batches, 72 trials total, 0 failures — but `mechanism=BOTH` (the double-fire signature) never reproduced in either batch, including `delay=0ms`, which should most favor it. Harness's wall-clock delay model likely doesn't track the fade's real in-flight state at dismiss time — open question, not resolved |
