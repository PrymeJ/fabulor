...
| `Investigation_260802_double_fire_reentrancy.md` | 08-02 | Investigation | Root-caused the gutter-dismiss double-apply: `snap_theme_forward`'s fallback is gated on `_fade_overlay.isVisible()`, which `.stop()` never clears (only `_on_fade_finished` does, and `.stop()` emits no `finished`) — 10/12 in a real session log. **Cost-only, not wrong-final-state.** Built an independently-verified live harness (pixel + stylesheet ground truth, never internal flags); found and fixed a harness bug first (sample point `(15,15)` landed on a title-bar glyph, not `bg_main` — caused a uniform 100% false-fail). Corrected batch 1: 0/36 failures, and the double-fire itself (`mechanism=BOTH`) didn't reproduce at any of the 36 swept conditions — open question for batch 2 |
