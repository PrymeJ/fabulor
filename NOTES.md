...
## Playing-seek oscillation — measurement round 1: did NOT reproduce; exposed a latent VT _seek_target coord-space bug (2026-06-15)

First `[SEEK-TL]` instrumentation pass (temporary, in working tree — NOT committed). Logged, while
`_seek_target` set + 1.5s after: raw `value`, `global_value = value + _file_offset`, `_seek_target`,
`dist`, `is_seeking` before/after, `pause`, the mode-aware walked chapter, and
`threading.current_thread().name`. Plus `[SEEK-TL-UI]` (computed chapter-slider value per 200ms tick)
and `[SEEK-TL-THREAD]` at `_on_file_loaded`. Snapshot: `/tmp/fabulor_seektl_session1.log`.

**Finding 0 (mandate 6 — SETTLED): `_on_time_pos_change` and `_on_file_loaded` run on
`MPVEventHandlerThread`, NOT the Qt main thread** (1209/1209 samples). So ANY QTimer added to the
settle path MUST marshal `start()/stop()` to the main thread (`QMetaObject.invokeMethod(...,
QueuedConnection)`) — plain attribute writes can stay off-thread. This is non-negotiable for any
future fallback-timer design.

**Finding 1: the playing bounce / chapter-slider-shoots-to-end did NOT reproduce this session.**
- Lotus Shoes (M4B) playing Next/Prev across chapters 28→29→30→32→34: every seek settled at
  `dist=0.0` (mpv landed exactly on target), chapter resolved correctly and stably each step, no
  bounce. Paused nav settled at the documented `dist=0.37` undershoot, also correct chapter.
- Doctor Zhivago (VT) playing nav: `[SEEK-TL-UI]` slider values climbed monotonically (chap 193
  sliderval 2→20, clean transition to chap 194 sliderval 0→1). No bounce in the UI either.
- User corroborated "seemed fine" on the short test. **The hypothesis behind the (unbuilt) settle-gate
  fix is NOT confirmed.** Do NOT build that fix until the bounce is actually captured in a log.

**Finding 1a (user lead, why round 1 was clean): the oscillation may be TIME/DRIFT/STATE-dependent** —
files "behaved at first but oscillated a few minutes later." Round 1 was a short test. Next round is a
LONGER soak across multiple files; user stops the instant oscillation appears and we read the log at
that point. The bug likely needs accumulated state or elapsed time, not just any playing seek.

**Finding 2 (NEW latent bug — VT `_seek_target` coordinate-space mismatch):** in VT sessions the log
shows settle samples like `value=0.000 global=64147.306 target=0.35 dist=64146.956 chap=194`. Here
`_seek_target` is stored as a small value (`0.35`, `0.1`) — local/raw space — while the settle
compares it against `global_value = value + _file_offset` (huge). `abs(global − target)` is enormous,
so the `< 1.0` settle in `_on_time_pos_change` NEVER clears `is_seeking` for these VT seeks — VT
relies entirely on the separate `file_switched` → `_on_vt_file_switched` path to clear the flag. It
works today by accident (the other path saves it), but the two `_seek_target` spaces are inconsistent.
This directly violates mandate 4 (both walks/comparisons must share one coordinate space) and would
break any agreement-gate built on `_seek_target`. Likely origin: VT cross-file seek / chapter-list
click / Prev-to-0 storing `_seek_target` without adding `_file_offset`. Investigate before building
any settle-gate fix on `_seek_target` for VT. (`target=0.35`/`0.1` = the `+_CHAPTER_BOUNDARY_EPSILON`
/ floor from a nav resolving to a file start.)

**Finding 3 (user-observed, not yet isolated in log):** right-click on progress-bar notches is
unreliable (lands wrong/inconsistent). Minor vs the rest. Isolate the notch-seek path next.

**Status:** instrumentation REMAINS in the working tree (uncommitted) for the next soak round. Remove
before any commit. The settle-gate fix plan is on hold pending an actual captured bounce.

### FIXED (commit b6a4023) — drop the stale backward sample in global space (2026-06-15)

The bounce/stick is FIXED. Guard at the top of `_on_time_pos_change`: while `not is_seeking and
_seek_target is None`, if the GLOBAL position (`value + _file_offset`) regressed > `_STALE_BACKWARD_
TOLERANCE` (0.3) from `_last_global_pos`, drop the sample (don't update caches, don't walk). New
`_last_global_pos` field tracks the last accepted global pos (reset in `load_book`). **Global space,
not local, is load-bearing:** global pos only climbs (a VT file switch advances `_file_offset` so it
climbs across files), so a regression is unambiguously the stale artifact — a local-space compare
would treat a VT file boundary (`value`→~0) as backward and could cascade-freeze the whole new file.
Verified via instrumentation (since removed): 32 drops, all the genuine ~0.31-0.34s post-seek sample,
all paused, ZERO during VT switches, zero forward-playback false positives. The whole elaborate
settle-gate plan (agreement gate / `_seek_target_chapter` lockstep / QTimer / monotonic N-count) was
DROPPED — it solved a mechanism that didn't exist; the real fix is one guard. Lever if a VT boundary
ever drops on a file whose mutagen duration is off by >0.3s: it self-heals in global space (sub-second
hiccup, not a freeze) — raise the tolerance toward 0.4-0.5 or fix `book_files` durations; do NOT
re-examine the guard.

### ROUND 2 — BOUNCE CAPTURED. Root cause is a STALE BACKWARD time_pos sample after a clean settle (2026-06-15)

Reproduced in Lotus Shoes (M4B) playing nav after a few minutes of use (timing-dependent — hence
round 1 looked clean). The mechanism, proven across many occurrences in the log:

1. A playing Next seek lands **EXACTLY on target** (`dist=0.0`) and `is_seeking` clears correctly on
   that sample. **The settle is NOT too loose — the plan's branch-A hypothesis is DISPROVEN.**
2. Immediately AFTER the gate clears, mpv emits 1–2 **stale `time_pos` samples ~0.6s BACKWARD** of the
   landing (e.g. seek lands 19193.019 → next sample 19192.394 → 19192.497 → then resumes forward
   19192.702…). The backward samples fall in the PREVIOUS chapter.
3. The 200ms `_sync_chapter_ui` can fire on one of those stale backward samples: it walks to the
   previous chapter and computes `sliderval = (pos − prev_chap_start)/chap_dur ≈ 999` → the chapter
   slider **shoots to the right (chapter end)**. Next (correct, forward) sample → slider snaps back to
   ~0 → the visible **bounce**. Confirmed by `[SEEK-TL-UI]` rows with `sliderval` 978–999 each landing
   ~0.6s behind a just-sought boundary.

**This is branch B, but NOT the form the plan anticipated.** The plan's fix (agreement gate +
`_seek_target_chapter` lockstep + monotonic N-count + marshalled QTimer fallback) was designed for
"gate clears too early." The gate clears correctly; the defect is a stale sample arriving *after* a
correct clear. **That entire design is mooted.** The real fix is small and targets the actual defect:
**reject a backward `time_pos` jump while PLAYING** — playback never legitimately moves backward, so a
backward sample during `pause=False` is a stale mpv artifact and must not drive the chapter UI (or the
position/sliders). Candidate sites: a guard in `_sync_chapter_ui` (and `_sync_progress_sliders`) that
skips a tick whose `pos` regressed > small threshold from the last rendered pos while playing; OR
reject the stale sample at source in `_on_time_pos_change` (don't update `_cached_time_pos` /
chapter-walk on a backward jump while playing). Must NOT suppress legitimate backward motion from a
user seek/Prev (those go through `is_seeking`/`_seek_target`, so gate the reject on
`not is_seeking and playing`). Mandate 6 (off-main-thread) still applies if any timer is used, but the
likely fix needs no timer at all.

Snapshot of the round-2 capture is in `/tmp/fabulor_run.log` (and round 1 in
`/tmp/fabulor_seektl_session1.log`).
