...
*Last updated: 2026-09-16 Session 1 — Fixed a separate undo-anchoring bug, live-verified by Pryme:
...
instead. Every `app.py` call site now calls `MainWindow._trigger_undo` unconditionally (no more
caller-side distance gate); call sites that never had one (long-skip, restart-to-0, chapter-slider
wheel) pass `threshold=0.0`. New `tests/test_undo_position.py` (6 tests, no mpv/QApplication). See
the "Undo must anchor to a seek spree's start" rule under "Critical Architecture Rules" for full
detail. Commits `3fe85a0`, `693274f` (date-typo follow-up).
