...
- **Standalone analysis documents go in `review/`, named `Type_YYMMDD_topic.md`** — see `review/README.md` for the six types and the full rationale. NEVER write one to the repo root, and NEVER give one the same name as a root document. Three rules, each of which exists because it was broken:
  ...
  A `Snapshot_` document must also say what commit it describes, in its own header (see `review/Snapshot_260717_theming_state.md`, accurate at `5cfe3a3` and stale within two weeks) — otherwise it silently becomes a source of wrong beliefs.
