...
- **Standalone analysis documents go in `review/`, named `Type_YYMMDD_topic.md`** — audits, investigations, reports, data dumps, point-in-time snapshots. NEVER write one to the repo root. Whatever it concludes must also be summarized into NOTES.md/TODO.md/DEBT_INVENTORY.md and linked from there, because **an unreferenced document is an invisible one**: seven such files were written to the root in July 2026 and cited from no main doc, and one (`review/Audit_260720_theme_reach.md`) was forgotten for six weeks until it happened to be opened in the IDE — while a later session re-derived a worse version of its call-site inventory from greps, unaware that it and three closely-related perf reports already existed in `review/`. All seven were moved and renamed 2026-08-02; `DEBT_INVENTORY.md` indexes them with status. A point-in-time snapshot must say so in its own title/header (see `review/Notes_260717_theming_current_state.md`, accurate at `5cfe3a3` and stale within two weeks) — otherwise it reads as current and silently becomes a source of wrong beliefs.
