...
### DO NOT remove the `_pre_switch_slider_value is None` guard from `_sync_progress_sliders` — and do NOT add the same guard to `_sync_chapter_ui`
`_sync_progress_sliders` guards `progress_slider.setValue` with `self._pre_switch_slider_value is None`. This blocks the 200ms timer from writing `pos≈0` in the window between `_on_library_hidden` (sets `_mpv_ready=True`, starts playback) and `_on_file_ready` (sets up the flow animation). Without it: `load_book` sets `_is_seeking=True` with no `_seek_target`; the first `time_pos=0` clears `_is_seeking`; the timer then writes 0 to the slider — visible 0% flash before the flow. The asymmetry with the chapter slider is intentional: `_on_file_loaded_populate_chapters` computes its animation target as `self.chapter_progress_slider.value()`, so the chapter slider MUST be written by the timer before that method runs. Adding the same guard to `_sync_chapter_ui` would make `new_chap_val == pre_chap` and kill the chapter flow animation.
