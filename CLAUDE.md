...
They are two independent soft-delete flags on `books`. `is_deleted = 1` is set by `remove_scan_location` (location removed from scan list). `is_excluded = 1` is set by `set_book_excluded` (user explicitly removed a book via the trash button) and by `mark_books_missing` (force-rescan missing detection). `is_deleted` resets to `0` in the `upsert_book`/`upsert_books_batch` ON CONFLICT blocks; **`is_excluded` is sticky and does NOT reset on upsert** (changed 2026-06-27 — `is_excluded=CASE WHEN books.is_excluded THEN 1 ELSE 0 END`, see the rule below). Stats queries are intentionally unfenced by both flags — listening history and progress survive removal permanently.

**Scanner resurrection behaviour:** `scanner.py` builds `known_paths` from `get_all_book_paths()` (unfenced — all rows regardless of flags). Excluded/deleted books are therefore recognised as known and skipped during non-force scans, so they are NOT automatically resurfaces. A force rescan (`force_refresh=True`, triggered by the Rescan button) re-processes all paths and calls `upsert_books_batch`, which resets `is_deleted` to 0 (resurrecting location-removed books) but **keeps `is_excluded` sticky** (changed 2026-06-27 — a force rescan no longer un-excludes a user-trashed or missing-flagged book; see the "Sticky `is_excluded`" rule below). Do NOT change `known_paths` to use `get_all_books()` or any fenced query — doing so caused excluded books to be silently resurfaces on every scan (2026-06-06 bug).

**Sticky `is_excluded` (2026-06-27):** Both upserts use `is_excluded=CASE WHEN books.is_excluded THEN 1 ELSE 0 END`, NOT a bare `is_excluded=0`. An excluded book (user-trashed via the detail trash button, or flagged missing by the force-rescan missing detection) stays excluded across any number of rescans. The ONLY restore paths are `set_book_excluded(path, False)` — called from the new **Excluded Books** section in the Library settings tab (`ui/excluded_books.py`, driven by `db.get_excluded_books()`), and from `restore_books_under_path` indirectly is NOT one (it only touches `is_deleted`). This deliberately reverses the prior "a force rescan resets both flags" behavior: the Excluded Books UI replaced rescan-as-restore. Keep both upserts in lockstep on this (the "DO NOT keep upsert_book and upsert_books_batch out of sync" rule covers it).

**Scanner missing-book detection (force rescan only, 2026-06-26):** A force rescan also *creates* `is_excluded=1` rows (it is no longer purely additive/resurrective). After Phase 1, for each location whose root `exists()` (`walked_locations`), `ScannerWorker.run_scan` diffs `db.get_visible_book_paths_under(loc)` (currently-visible books, `is_deleted=0 AND is_excluded=0`) against the folders rediscovered on disk and calls `db.mark_books_missing(paths)` (batch `is_excluded=1`) for any visible book whose folder is gone. This is the same `is_excluded` semantics as `app.py`'s `_mark_book_missing` (book stays in DB/stats, resurrectable by a later force rescan if the folder returns), just batch and scanner-thread-side. Two guards are load-bearing and must survive any refactor: (1) it runs ONLY on `force_refresh=True`, never on non-force scans; (2) it is scoped to `walked_locations` ONLY — an offline/unmounted location (root `exists()` False) is never in that list, so its books are NEVER falsely flagged when its drive is detached. The inner per-folder `entry.iterdir()` audio check is wrapped in `try/except (PermissionError, OSError)`; skipped folders accumulate in a function-scoped `skipped_dirs` set (initialized once at the top of `run_scan`, only `.add()`ed, never reassigned) that is folded into the `discovered` set, so a transient per-folder I/O error never reads as "folder gone". Do NOT scope `skipped_dirs`/`walked_locations` inside the loop or the except block.
