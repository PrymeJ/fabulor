...
cut to 55%, value +8/255 clamped) — deliberately NOT the same value boost as
StreakGrid._derive_longest_fill's +60/255, which read as too bright when
tried live as a whole-button fill (2026-09-08 live report, tuned down across
three passes: +60 -> +25 -> +12 -> +8). See the function's own docstring in
themes.py for the full rationale.
"""
...
def _reference_derive(accent_hex: str) -> tuple[int, int, int]:
    ...
    the function under test — same sat*0.55/value+8/255 design, computed via
...
def test_derive_lighter_accent_rgb_is_less_boosted_than_streak_grid():
    ...
    for accent_hex in ["#ff6b35", "#3a7ca5", "#8e44ad"]:
        ...
        # +8/255 boost should land strictly below a +60/255 boost would, on
