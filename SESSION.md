## Session Summary — 2026-08-04 Session 2 — `investigate/restyle-cost-depth-and-narrowing`: the actual root causes behind three days of hover-leak reports, found and fixed — write-path confinement, the `_apply_stylesheets` hover-exclusion fix, and corrected snapback timing

Continues directly from Session 1's `get_displayed_theme()` redirect (`5630cf0`). Live testing the
same day found that redesign fixed WHAT a theme read returns but not WHO was reading it from where —
three further, genuinely distinct root causes were found and fixed, each one narrower and more
precisely targeted than the last, after Pryme repeatedly rejected reactive "catch up and correct"
fixes and insisted on finding why a wrong value could be observed at all.

**Fix 1 — write-path confinement (`7c4682f`).** Live testing found `LibraryPanel._resolve_theme_
colors`/`_refresh_search_match_state` and Speed/Sleep's preset-ramp color functions calling
`theme_manager.get_current_theme()` directly from ordinary UI events (a view-mode click, typing in
search, a preset-button click) — none of which have any relationship to a theme change, and all of
which are structurally invisible while any hover is even possible (Settings and Library/Speed/Sleep
are mutually exclusive panels). Since `get_current_theme()` now resolves through the hover-inclusive
`get_displayed_theme()`, any of these unrelated events could bake a live hover into a panel the user
wasn't even looking at — confirmed live at up to 78 seconds of staleness. Pryme named the actual
invariant directly: *"Anything invisible has no business calling anything to get a new color."*
Required-first-step check confirmed `_current_theme_name` was already exactly the value needed —
written only by four deliberate-commit sites (app-launch pool pick, rotation/"Change now", a
pool-edit edge case, right-click), never by any hover path (`_mark_theme_applied`'s own docstring
confirms it is the sole writer of the hover-related fields and never touches this one). Fix: one new
accessor, `get_committed_theme()` (returns `_current_theme_name` directly, no new state), and six
leak sites — re-audited fresh rather than trusting the list assembled hours earlier, which had
already missed one site — redirected to it. Verification is synthetic by necessity: a new CLAUDE.md
rule this same day formalized that Settings and every other panel are mutually exclusive by
construction, so a live click sequence can never distinguish "the fix worked" from "the hover had
already ended for an unrelated reason" — 8 new tests force the hover fields directly and assert each
fixed consumer resists them. Suite: 450/450. Full detail:
`review/Design_260804_write_path_confinement.md`.

**Fix 2 — `_apply_stylesheets` excludes Speed/Sleep from hover entirely (`f2cfa6d`), the fix that
finally closed the loop.** Pryme re-tested fix 1 live and found Speed/Sleep's PANEL BACKGROUND
(distinct from the ramp buttons fix 1 already corrected) still showing hover colors — and pushed
back hard on the pattern: *"I get 'oh, this was a pre-existing bug'... The reason I asked for a new
design is that pre-existing bug... I am trying to get rid of the redundant 'let's style all the
panels, so there wouldn't be anything stale.'"* Traced from a live repro (Highgarden active,
"Midnight Children" hovered, panel closed within ~300ms) to the exact mechanism: a genuine leaveEvent
was misclassified as a blur-grab synthetic by `_on_themes_tab_left`'s hidden-widget branch
(`[SWATCH-LEAVE-SUSPECT]` correctly detected this but only ever logs, never corrects), leaving
`_is_hover_active` stuck `True` for 90+ seconds — during which `_apply_stylesheets` kept
unconditionally overwriting `_pending_panel_sheet`'s stash for ALL three of settings/speed/sleep,
regardless of hover correctness. This is why Library/Stats/Tags were "always correct" — not cleverer
code, just structurally never reached by this call at all (Library re-reads fresh via `get_committed_
theme()`; Stats/Tags/Book-Detail get the confinement-clean `theme_applied` signal) — while Speed/
Sleep's background had no such immunity. Fix, per Pryme's own explicit framing: *"Style whatever the
fuck is visible, stay off from other panels."* `panel_sheets` now omits `speed_panel`/`sleep_panel`
entirely when `hover=True` — omission, not correction, the same shape that already makes Stats/Tags
correct. The stash write changed from a bare-replace `dict(panel_sheets)` to `dict.update(...)`,
since a hover's narrower dict would otherwise delete Speed/Sleep's existing correct entries outright,
which would have been a worse regression than the bug being fixed. Self-contained: does not touch
`_on_themes_tab_left`, `[SWATCH-LEAVE-SUSPECT]`'s detect-but-don't-correct gap (still open, still
logged, explicitly deferred — not this session's to fix), the confinement guard, or `get_committed_
theme()`. Verified via 3 new tests against a real headless `QApplication`/`ThemeManager` (not fakes,
since the bug lived in exactly how the stash dict itself is built) — suite 453/453. Pryme re-tested
live afterward and confirmed containment, cross-checked against the log
(`[PANEL-SHEET-STASH]`/`[PANEL-SHEET-CATCHUP]`) to verify the fix's actual mechanism rather than just
trusting the visual result. Full detail: NOTES.md "The actual root cause."

**A genuinely separate, explicitly out-of-scope observation, recorded not investigated:** Pryme
noticed that loading a book from Library appeared to "restore" correct theme colors during a stuck-
hover repro, and flagged this as suspicious in its own right — a book load should have no
relationship to theme state unless cover-art-based theming is active. Explicitly NOT the same
question as the no-book-state theme touch (confirmed legitimate and out of scope — background-image
themes need special handling there). Recorded in TODO.md as its own unverified item, not traced this
session.

**Fix 3 — corrected snapback timing for Esc/gutter-dismiss (design doc + implementation, this
session).** A further investigation task confirmed a fourth, distinct gap: even with hover-leaks
fully confined, a fast dismiss during an active hover could close Settings before the revert had
visibly settled. Traced precisely: `_on_theme_unhovered()`'s snapback (`hover=False`, not a
selection) was excluded from `_on_theme_changed`'s `_hover_may_interrupt` predicate, so a hover-out
arriving while the original preview's own fade was still running got STASHED behind it — resumed only
once that fade finished, up to the user's configured preview duration (as long as 1500ms).
`_close_settings_flow`'s call ordering was already correct; the gap was entirely that a stashed
snapback doesn't apply until its blocking fade ends. A mandatory backstop-timer double-fire check
(explicitly required by Pryme's task) found a real, previously-benign race in the stash window,
closed as a side effect of the interrupt fix rather than needing its own guard. Fix: `_snapback_in_
progress` (mirrors the existing `_selection_in_progress` pattern exactly) widens the interrupt
predicate; `ThemeManager.call_when_theme_settled` (mirrors `PanelManager.call_when_panels_settled`'s
predicate-driven, non-`finished`-signal shape exactly, for the same documented reason — `.stop()`
doesn't reliably emit `finished` on this codebase) blocks `_close_settings_flow`'s slide-out until the
revert has genuinely settled, plus a small tunable settle gap (150ms) paid only when a real hover-out
occurred; a re-entrancy guard (found necessary during implementation, not in the original spec) stops
a spammed dismiss from re-issuing the revert mid-wait. **Settings-internal tab-switch was
investigated and confirmed structurally different — deliberately NOT implemented.** `currentChanged`
fires after Qt has already switched tabs, with no pre-change signal to block from; a true block needs
a new tab-bar event-filter mechanism. Pryme's explicit call: verify the dismiss fix in isolation
first, treat tab-switch as its own separate follow-up, so a regression in one is never conflated with
a regression in the other. Verified via 12 new synthetic tests across three files, mirroring existing
settle-watch/interrupt-predicate test patterns exactly; suite 465/465; live smoke test clean. Full
detail: `review/Design_260804_snapback_timing.md`.

**Every fix this session was preceded by an explicit, code-level trace before any change was
proposed** — per Pryme's own repeated, direct instruction against reactive patching, most sharply
stated after the write-path-confinement fix: *"It should have been checked and confirmed that the
new design doesn't style the wrong panels since it was the main goal of the design in the first
place."* Nothing in this session's three fixes touches `_on_themes_tab_left`'s leaveEvent
misclassification itself, `[SWATCH-LEAVE-SUSPECT]`'s detect-but-don't-correct gap, or tab-switch
blocking — all three remain open, explicitly named, and deliberately deferred rather than silently
left unaddressed.
...
patched the same underlying defect reactively — `_is_hover_active`/`_active_display_theme_internal`
are stored fields whose correctness depends on a `leaveEvent` being correctly classified, and that
exit contract had already failed via three distinct mechanisms in one evening. Pryme rejected a
fourth reactive patch outright ("The whole thing is wrong. The solution is not trying to revert the
damage, it is to prevent the damage from happening") and directed a structural redesign instead.

**Design (`review/Design_260804_hover_state_computed_read_path.md`).** Replace "is hover active, and
what theme" as stored state with a question re-asked from scratch on every read (`get_displayed_theme()`):
is Settings visible, is the Themes tab current, is the cursor over a specific swatch right now — three
live Qt/OS facts that cannot go stale because nothing is cached. Scope confirmed cleanly separable
from the fade/animation-triggering pipeline: the one place `_is_hover_active`/`_active_display_theme_
internal` are read *inside* `_on_theme_changed` is a write-side dedup check, not an external "what
should a reader see" query, so the two stored fields stay exactly as-is for the fade pipeline's own
bookkeeping while every external reader gets redirected to the new method. All 9 real runtime call
sites (2 more than the prior session's audit found, both already routed through the "sanctioned"
accessor) audited and confirmed correct with zero call-site changes needed.

**Staged rollout, four steps, each independently verified before the next was authorized** — Pryme's
explicit requirement, citing this exact branch's own track record of reactive changes that only got
caught live rather than verified in advance:

1. **Add `get_displayed_theme()`, purely additive** (`0abe9ac`) — nothing calls it yet.
2. **Shadow-check only**: `get_active_theme()`'s existing body gets a log-only comparison against
   `get_displayed_theme()`'s answer, without changing what's returned (`52ec0fd`). Extended same day
   with a running call/disagreement counter + periodic summary, and a `[SHADOW-CHECK]`-filtered
   stdout mirror (a `StreamHandler` on `theme_manager.py`'s own child logger, not the app's
   file-only root logger) so disagreements were visible live without tailing the log file.
3. **Live verification across real sessions**, spanning hours and multiple distinct testing
   patterns (plain hovering, right-click selection + hover, Exclusive/with-pool cover-theme toggling
   under active theme rotation with blur on). A **resting-hover probe** (`_resting_hover_probe_timer`,
   700ms, temporary) was added mid-session after discovering none of the 9 real call sites poll on
   any timer — a quiet resting hover produced literally zero `get_active_theme()` calls, so silence
   there was never evidence of agreement, only evidence the question was never asked. The probe alone
   pushed one session past 400 calls with a full, decomposed accounting of every disagreement.
4. **Redirect** (`5630cf0`, marked wip pending Pryme's continued testing): `get_active_theme()` now
   returns `self.get_displayed_theme()` directly. All shadow-check/probe scaffolding removed in the
   same change. `get_current_theme()` needed no edit — its existing one-line delegation inherits the
   new behavior automatically. `check_cursor_on_settle`, `clear_stale_hover_state`, and the three
   fade-pipeline-internal mechanisms (`_is_hover_active`, `_active_display_theme_internal`,
   `_check_swatch_still_hovered`) were explicitly NOT touched — their deletion/disposition remains a
   separate, later step per the design's own sequencing.

**Five distinct disagreement mechanisms found and precisely diagnosed during step 3**, not lumped
together despite superficially similar log shapes — see the design doc §7-§7e for full detail:
an Esc-dismiss ordering race (state reverts before the panel widgets report themselves as closing);
a debounce-vs-instantaneous-read mismatch during cursor sweeps AND right-click selections (the
80/150ms hover debounce hasn't committed, while the new read has no debounce at all); the OLD code's
own deliberate hover-concealment contract (`_is_hover_active` branch always hides the preview from
external callers — not a defect, this is the exact behavior the redesign exists to retire); and a
genuinely new, pre-existing correctness gap — `get_active_theme()`'s non-hover branch never checked
`_cover_theme_active` at all, so a cover-theme apply stashed behind an in-flight fade could return a
stale theme name for up to 750ms, to every one of the 9 call sites, independent of hovering.

**Two of my own conclusions were wrong mid-session and corrected on the record rather than silently
fixed** — both caught by Pryme, not self-caught:
- The original call-site audit claimed a "book-load while hovering" scenario was the single most
  important case to verify. Pryme corrected this directly: Library and Settings/Themes can never be
  open simultaneously (`PanelManager.is_overlay_open_or_committed()`'s one-overlay-at-a-time gate),
  so the scenario is structurally impossible, not merely rare — dropped from the verification
  checklist entirely.
- Trying to explain why `[SWATCH-LEAVE-SUSPECT]` couldn't be deliberately reproduced despite several
  minutes of trying, I first floated (wrongly) that the underlying hide/show mechanism might have
  been removed — retracted on further reading (it's still there, `transport_bar_blur.py:941-942`).
  My second explanation ("faster tick cadence makes the hidden window shorter, harder to land in")
  was also wrong — Pryme pointed out this conflates tick FREQUENCY with per-tick hidden-window
  DURATION, two different quantities. Checked precisely: the ~48ms hidden-duration figure is a
  quiet-playback steady-state baseline from the code's own comments; the bursty ~15-20ms-apart
  samples pulled from an active-interaction window happened because `refresh_dirty` is coalesced off
  real Qt Paint events, not polled at any fixed rate — there is no "refresh rate" to lower, but
  whether that many distinct dirty events during active interaction each genuinely need their own
  grab-and-blur is a real, separate, still-open question, recorded for whoever next touches
  `transport_bar_blur.py`'s dirty-tracking granularity.

**Step 4 landed and was confirmed live by a real, unstaged occurrence of the original bug** —
Pryme reported seeing "Sunspear" in the main window while "Eyes of Ibad" was the actual active theme
during ordinary post-deploy use, reverting later. Log correlation confirmed this happened under the
NEW code (app instance running since before the episode), with `_active_display_theme_internal`
genuinely stuck for ~9 seconds (a real, live instance of the pre-existing leave-misclassification bug
this whole redesign targets — caught and corrected by the pre-existing, untouched
`_check_swatch_still_hovered` backstop) while `get_displayed_theme()`'s geometry guard — confirmed by
direct code reading against the exact `[SWATCH-BACKSTOP-COST]` visibility/position samples logged
during the episode — never had a path to return the stale value to any external caller throughout.
Pryme confirmed live that the visible correction happened well before the bookkeeping settled,
consistent with the paint tracking the backstop's correction rather than the stale field.

**One related, explicitly out-of-scope UX issue surfaced and deferred to TODO.md, not fixed**: Pryme
wants a Themes-tab hover preview's colors confined strictly to "while genuinely hovering AND on the
Themes tab" — settling the snapback to the real active theme BEFORE switching to another Settings tab
or dismissing the panel, not after (the transition should never visibly carry the preview color with
it, even briefly). Scoped as a follow-up for after this redesign fully lands, likely one fix (force/
await the snapback's completion at the point of leaving the Themes-tab context) applied at two call
sites (a Settings-internal tab-change handler, and `PanelManager._close_settings_flow`'s existing
`_on_theme_unhovered()` call).

**Full suite green throughout** — 442/442 at every one of the four steps, matching baseline exactly,
no regressions introduced at any stage. `check_cursor_on_settle`/`clear_stale_hover_state` deletion
and any further live testing (item 7 of the design's own verification checklist — confirming ordinary
Themes-tab preview UX is unchanged) remain open, explicitly deferred to a later session at Pryme's
own pace.
...
the user against the running app, watched via `[BLEED-TRACE]`/`[FADE-FINISHED-TRACE]` logging): a
theme-hover snapback (`_on_theme_unhovered`) could get stuck showing the hovered theme indefinitely
if the mouse left while its preview fade was still animating AND the Settings/Themes panel was still
open. Cause: `_pending_fade_call`'s stash tuple silently dropped `bypass_panel_open_guard` (the flag
`_on_theme_unhovered` always sets `True` so its snapback can apply even with a panel open), so every
replay site (`_on_fade_finished`, `snap_theme_forward`, `complete_main_fade`) resumed with the
default `False` — landing the replay in the panel-open guard branch and queuing it into the
single-slot `_panel_guard_timer`, which then kept getting clobbered by ongoing hover activity.

Before implementing, confirmed an explicit exclusivity requirement (no call site ever passes
`hover=True` and `bypass_panel_open_guard=True` together — checked both statically across every
`_on_theme_changed` call site and empirically against 65 live-observed `hover=True` calls, zero
exceptions), so widening the stash to a 6-tuple can't reopen the hover-preview-confinement bleed the
2026-07-21 discard rule exists to prevent. Fixed, pytest clean (only the 4 pre-existing
`test_cover_theme_pending.py` failures), live-verified: Repro A/B (straight-out and
via-neighboring-swatches leave-mid-fade) both now snap back correctly. `8243959`.

**Live verification surfaced a second, unrelated stuck-hover bug** (not caused by this fix — confirmed
via `git diff`, the fix touched only 3 scoped lines): with blur enabled, a genuinely still, deliberate
hover can silently never convert to an applied preview. Root-caused precisely: `themes_tab.leaveEvent`
(a bare lambda, no synthetic-leave suppression, unlike `ThemeItem`'s own careful handling from the
