...
## DO NOT reintroduce loose book-switch flags on MainWindow, and DO NOT bypass the book-switch state machine. The book-switch transition is owned by ONE object: self._switch, an instance of BookSwitchState in src/fabulor/book_switch.py. It holds every switch-specific flag — the deadzone (in_deadzone, was _mpv_ready), the captured pre-switch slider values (flow_pending_progress / flow_pending_chapter), the duration-retry flag (chaps_dur_retried), and the deferred-handler flags (file_ready_deferred / chaps_deferred). There must be NO equivalent attribute living directly on MainWindow ever again — adding self._mpv_ready, self._pre_switch_slider_value, self._pre_switch_chap_slider_value, or similar back to app.py is forbidden. Read state through the named predicates (self._switch.in_deadzone, .flow_pending_progress, .flow_pending_chapter); change state only through the transition methods (begin(), library_revealed(), take_progress_target(), take_chapter_target(), mark/clear_*_deferred()). If you think a new book-switch flag is needed, it goes ON BookSwitchState, not on MainWindow — and you must say so explicitly and wait for confirmation before adding it.

## DO NOT fold the orthogonal seek/drag/animation guards into BookSwitchState. The state machine owns ONLY switch-specific state. These are NOT switch-specific and must stay exactly where they are: player._is_seeking / player._seek_target (the generic seek gate, also used by chapter navigation and manual slider seeks), the slider-drag flags (is_slider_dragging / is_chapter_slider_dragging), the _flow_anim running-state checks, and player.mp3_seek_reload_pending. They fire for reasons that have nothing to do with book switching (chapter nav, manual seeks, theme color animations, MP3 stop-and-load reloads) and they are the documented fixes for separate bugs (chapter-label VU-meter oscillation, Prev/Next snap-back, the 0% flash, seek jitter). Absorbing any of them into the state machine WILL reintroduce those bugs. The correct pattern is to COMPOSE — a guard reads both, e.g. `not self.player.is_seeking and not self._switch.flow_pending_progress`. Removing either half is forbidden.

## DO NOT peek a BookSwitchState pre-value by consuming it, and DO NOT rely on take_*() for a side effect. take_progress_target() and take_chapter_target() are consuming reads: each captured value can be read exactly once, and that read is precisely what tears the switch down (it flips flow_pending_* to False). NEVER call take_*() merely to look at the value, and NEVER read-then-restore it. If a future change genuinely needs to inspect a pre-value without consuming it, add a NEW non-consuming peek property to BookSwitchState (e.g. pre_progress / pre_chapter) — do not work around the consume-once contract. Say so explicitly and wait for confirmation before adding the peek property.
...
## DO NOT restore any emit in `_on_chapter_change` — it is fully suppressed as of 2026-06-01. `_on_chapter_change` now contains only `return`. `_on_time_pos_change` drives `chapter_changed` universally for all book types via position walk (VT: walks `_chapter_list` against global pos; CUE: walks `_chapter_list`; embedded M4B: walks `self.instance.chapter_list`). The old `_is_seeking` guard on `_on_chapter_change` was insufficient: `_on_time_pos_change` clears `_is_seeking` first, so `_on_chapter_change` always fired with `_is_seeking=False` and emitted the stale mpv native chapter value, causing visible chapter label snap-back on Prev/Next seeks while paused.
