...
- **[2026-06-22, originally 2026-06-21] StreakGrid left-gutter date labels clip their first letter
  ("Jan 01"/"Aug 28"/"Sep 28" -> "an 01"/"ug 28"/"ep 28").** Confirmed via a 12-month offscreen
  sweep (2026-06-22) that this is NOT "J"-specific — most months lose their first letter the same
  way; "Jul" was the one exception observed. (The descender-clipping half of this bug, e.g. "g" in
  "Aug" getting cut off vertically, was a separate, already-fixed issue — see NOTES.md 2026-06-22
  entry.) `AlignRight`-anchored "Mon DD" text at 9pt is wider than the 29px gutter rect; the overflow
  is normally invisible (side-bearing whitespace) but enough first-letter ink sits in that overflow
  band for most months to get clipped at the widget's left edge. See NOTES.md "HourlyHeatmap top
  date labels: 'J' clipped" (2026-06-21) for the four approaches tried and rejected (widening the
  rect pushes the other end into the cell grid; 8pt alone still clips; 7pt fixes it but was rejected
  as illegibly small with too much dead gutter whitespace). **Blocked on:** a format/layout idea
  that keeps the label readable at a normal size without widening past `GUTTER_W` (shared,
  fixed-size with `HourlyHeatmap` for the tab transition). Candidate not yet tried: numeric `M/D`
  format (shorter; digits don't have the same ink-outside-advance-box problem) — flagged to the
  user, not yet decided on.
