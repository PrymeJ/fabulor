...
- [2026-08-02] **ROOT CAUSE FOUND:** ANY `mw.setStyleSheet()` call costs ~550ms regardless of argument — full sheet, 1 rule, identical string, and EMPTY string all measure the same; Qt does not no-op an identical sheet and clearing is not cheaper. DEPTH is the multiplier, not widget count. **(F) is dead** (emptying the root buys nothing — the 8ms figure was a measurement error, corrected). **(E)** skip the root call when the string is unchanged is the only surviving non-structural option, but 12 themes give 12 distinct sheets so it does NOT help previews — only repeat applies of the same theme. Real fix is depth reduction → Stats refactor. See NOTES.md 2026-08-02
