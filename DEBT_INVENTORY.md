...
- **Diacritic-folded sort (library + tags) is a locale-agnostic approximation, not real
  ...
  diacritic letter as its own distinct letter rather than a decorated variant. Accepted as a
  permanent simplification, not scheduled work — real per-language correctness would need
  locale-aware collation (ICU/Python `locale`) pinned to a specific language, and there's no
  single right answer across languages to pin it to. See CLAUDE.md "Pending / Known Debt".
...
**Where these documents belong: `review/`, named `Type_YYMMDD_topic.md` — see `review/README.md`
for the six types and the naming rules.** Seven files from the
