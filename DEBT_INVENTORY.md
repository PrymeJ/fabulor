...
- **One dead pixel per row in Stats/Tags — clicks ignored, arrow cursor** (2026-07-31) — the last pixel of each row (row-local y=51 of 52) is delivered to the rows container instead of the row; `childAt()` returns the row for that pixel while Qt routes the press elsewhere. Tinting proved the rows are flush and the pixel is painted by the row, so it is a hit-test/paint mismatch, not a layout gap. Mechanism unidentified; six causes ruled out with measurements. See TODO.md and NOTES.md, 2026-07-31.
