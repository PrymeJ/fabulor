...
- **"1 per row" right-aligned time/progress drift with the scrollbar** (2026-07-06) — same class of bug as the List-mode one just fixed: 1-per-row's total/remaining time + progress are right-aligned and derive their x from the live viewport width (`option.rect.width()`), so they shift ~14px when the vertical scrollbar appears/disappears on filter. List mode was fixed via `_list_content_width` (reserve the scrollbar gutter unconditionally); 1-per-row was left for a follow-up. Fix: route its width through the same stable-width reservation (`view.width() - 2*frameWidth - SCROLLBAR_EXTENT`), don't read `option.rect.width()`. See the CLAUDE.md rule "DO NOT lay out a library row from the live viewport width".
...
- **List-mode row geometry depends on live `option.rect.width()`, which is not constant** (2026-07-06) — `BookDelegate._paint_list_row` reads `option.rect.width()` to derive `AVAILABLE` and everything downstream, but `setResizeMode(ResizeMode.Adjust)` + `ListMode` makes the row width track the live viewport (not a fixed `sizeHint`), so a paint mid-slide-in / pre-settle can differ → the same title elides in one paint and not another. Resting geometry is otherwise a pure function of `(book, option.rect, option.font)`; width is the only non-constant input. (The scrollbar-toggle case specifically is now handled for List via `_list_content_width`; this entry covers the residual mid-transition/pre-settle instability.) See NOTES.md "List-mode title/author spacing" (orthogonal-instability section). hover-invade only triggers/holds while the cursor is in the author zone `[mid, right)`, but an expanded elided author draws leftward past `mid`, so its FIRST name sits left of `mid` — moving there leaves the zone and collapses it (can't be clicked, and can't even be fully read). Accepted limitation, inherent to pre-existing invade geometry, not specific to the click-to-filter feature that exposed it; the first name stays clickable in the resting/partly-elided state. Fix would widen the invade hold-zone to the full expanded rect, which competes with the title's left-side expand zone when both are elided — deferred. Comment at `_list_author_layout`'s `expand_author`. See SESSION.md 2026-07-06 (List author click-to-filter).
- **List-mode row geometry depends on live `option.rect.width()`, which is not constant** (2026-07-06) — `BookDelegate._paint_list_row` reads `option.rect.width()` to derive `AVAILABLE` and everything downstream, but `setResizeMode(ResizeMode.Adjust)` + `ListMode` makes the row width track the live viewport (not a fixed `sizeHint`), so a paint mid-slide-in / pre-settle can differ → the same title elides in one paint and not another. Resting geometry is otherwise a pure function of `(book, option.rect, option.font)`; width is the only non-constant input. Affects any List-mode geometry, not just title/author spacing. See NOTES.md "List-mode title/author spacing" (orthogonal-instability section). `BookDelegate._paint_list_row` reads `option.rect.width()` to derive `AVAILABLE` and everything downstream, but `setResizeMode(ResizeMode.Adjust)` + `ListMode` makes the row width track the live viewport (not a fixed `sizeHint`), so a paint mid-slide-in / pre-settle can differ → the same title elides in one paint and not another. Resting geometry is otherwise a pure function of `(book, option.rect, option.font)`; width is the only non-constant input. Affects any List-mode geometry, not just title/author spacing. See NOTES.md "List-mode title/author spacing" (orthogonal-instability section).
